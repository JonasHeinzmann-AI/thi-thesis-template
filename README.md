...
exempt. The author affirms that their own cognitive and creative contribution
predominates.

Two things about that file are worth knowing before you rely on it.

The source slide was marked as a preliminary version. It also drops the clause
...
template cannot do that for you. Requirements change over time.

## Word version

Some Prüfungsämter and some supervisors want a `.docx`. `word/thesis-template.docx`
is the same skeleton in Word: A4 with the 30/20 mm margins, 12 pt Times New Roman
at 1.5 spacing, real Heading 1 and Heading 2 styles, a page-number field in the
footer, plus the declaration of authorship with the same AI-usage paragraph.

The table of contents is a **field**, so it is empty until you build it: right-click
it and choose "Update Field", or put the cursor in it and press F9. That is
deliberate. A typed-out list of headings looks identical until the first edit and
then goes stale without telling you.

The document is generated by `word/make_template.py`, with both committed. A
`.docx` is a zip of XML, so a change to one cannot be reviewed in a diff; editing
the script and regenerating keeps the history readable.
...
word/                 make_template.py and the generated thesis-template.docx
```
...
copy rather than an authoritative source. Check it against the current official
version.
